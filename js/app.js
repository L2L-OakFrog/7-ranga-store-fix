...
/* const detailsInfo = (id) => {
  ...
  fetch(url)
            .then(res=>res.json())
            .then(json=>console.log(json)) // shows in console
  showDetails();
}
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button onclick="detailsInfo(${id})" id="details-btn" class="btn btn-secondary">Details</button></div>
      `;
...
const addToCart = (id, price) => {
  ...
  updateTaxAndCharge(); // called tax amount function
  updateTotal(); // called total amount function
